...
fn is_bundled_codex_path(path: Option<&str>) -> bool {
    let Some(path) = path.map(str::trim).filter(|value| !value.is_empty()) else {
        return false;
    };
    let Some(bundled_path) = crate::bundled_cli::bundled_cli_path() else {
        return false;
    };
    std::path::Path::new(path) == bundled_path
}
...
pub(crate) async fn codex_update_core(
    ...
    if resolved
        .as_deref()
        .is_some_and(|value| is_bundled_codex_path(Some(value)))
        || (resolved.is_none() && crate::bundled_cli::bundled_cli_path().is_some())
    {
        ...
        let result = CodexUpdateResult {
            ...
            output: None,
            details: Some(
                "Bundled Codex is managed by LadonX and is not updated via npm or brew."
                    .to_string(),
            ),
        };
