...
pub(crate) fn build_run_metadata_prompt(cleaned_prompt: &str) -> String {
    if contains_cjk_text(cleaned_prompt) {
        return format!(
            "你负责为编码任务生成简洁的运行元数据。\n\
只返回一个 JSON 对象，包含这些键：\n\
- title：简短清晰，3-10 个字，使用中文，并保留必要的技术名词\n\
- worktreeName：小写 kebab-case slug，必须以下列前缀之一开头：\
feat/、fix/、chore/、test/、docs/、refactor/、perf/、build/、ci/、style/。\n\n\
当任务是 bug 修复、错误、回归、崩溃或清理时选择 fix/。\
对于杂项、测试、文档、重构、性能、构建、CI、样式任务使用最接近的前缀。\
其他情况使用 feat/。\n\n\
示例：\n\
{{\"title\":\"修复登录跳转循环\",\"worktreeName\":\"fix/login-redirect-loop\"}}\n\
{{\"title\":\"添加工作区首页\",\"worktreeName\":\"feat/workspace-home\"}}\n\
{{\"title\":\"更新 Lint 配置\",\"worktreeName\":\"chore/update-lint-config\"}}\n\
{{\"title\":\"添加覆盖率测试\",\"worktreeName\":\"test/add-coverage-tests\"}}\n\n\
任务：\n{cleaned_prompt}"
        );
    }
...
pub(crate) fn build_agent_description_prompt(description: &str) -> String {
    if contains_cjk_text(description) {
        return format!(
            "你负责生成自定义编码 agent 的配置文本。\n\
只返回一个 JSON 对象，并且必须只包含这些键：\n\
- description：简短的角色摘要，一句话，4-12 个中文词或等量短语。\n\
- developerInstructions：多行 agent 指令。\n\n\
要求：\n\
- 保留用户意图，即使输入很短。\n\
- description 要简洁、实用，并使用中文。\n\
- developerInstructions 要可执行、具体，并使用中文。\n\
- developerInstructions 必须是 3-8 行。\n\
- 不要包含 markdown 代码块。\n\n\
示例：\n\
{{\"description\":\"调查不稳定测试并稳定测试套件\",\"developerInstructions\":\"优先调查不稳定测试失败并定位根因。\\n在提出修改前尽量稳定复现失败。\\n优先采用最小且安全的修复，并补充有针对性的回归覆盖。\"}}\n\n\
用户提示：\n\
{description}"
        );
    }
