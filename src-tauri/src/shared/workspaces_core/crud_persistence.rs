...
use crate::storage::{workspace_id_for_path, workspace_id_for_path_and_surface, write_workspaces};
...
fn workspace_path_key(path: &str) -> String {
    workspace_path_to_string(&normalize_workspace_path_input(path))
}

fn normalize_workspace_surface(surface: Option<String>) -> Option<String> {
    match surface.as_deref() {
        Some("claude_code") => Some("claude_code".to_string()),
        _ => None,
    }
}

fn workspace_surface_key(entry: &WorkspaceEntry) -> &str {
    entry.source.as_str()
}
    ...
    Fut: Future<Output = Result<Arc<WorkspaceSession>, String>>,
{
    ...
    let surface = normalize_workspace_surface(surface);
    if let Some(existing) = {
        ...
        workspaces
            .values()
            .find(|entry| {
                workspace_path_key(&entry.path) == path_key
                    && workspace_surface_key(entry) == surface.as_deref().unwrap_or("codex")
            })
            .cloned()
    } {
        let connected = sessions.lock().await.contains_key(&existing.id);
    ...
    let entry = WorkspaceEntry {
        id: workspace_id_for_path_and_surface(&path, surface.as_deref().unwrap_or("codex")),
        name: name.clone(),
        path: path.clone(),
        source: surface.clone().unwrap_or_else(|| "codex".to_string()),
    ...
    Fut: Future<Output = Result<Arc<WorkspaceSession>, String>>,
{
    ...
    let entry = WorkspaceEntry {
        id: workspace_id_for_path(&stored_destination_path),
        name: copy_name,
    ...
    Fut: Future<Output = Result<Arc<WorkspaceSession>, String>>,
{
    ...
    let workspace_name = clone_path
        ...
        .to_string();
    let entry = WorkspaceEntry {
        id: workspace_id_for_path(&stored_clone_path),
        name: workspace_name,
