...
use super::files::{
    list_directory_files_inner, list_workspace_files_inner, read_workspace_file_inner,
...
#[tauri::command]
pub(crate) async fn list_directory_files(path: String) -> Result<Vec<String>, String> {
    let root = crate::settings::expand_home_path(path.trim());
    ...
    Ok(list_directory_files_inner(&canonical_root, 20_000))
}

#[tauri::command]
pub(crate) fn current_workspace_path() -> Result<String, String> {
    let path = crate::settings::resolve_default_workspace_dir()?;
    std::fs::create_dir_all(&path).map_err(|err| {
        format!(
            "Failed to create default workspace directory {}: {err}",
            path.display()
        )
    })?;
...
pub(crate) async fn add_workspace(
    ...
    if remote_backend::is_remote_mode(&*state).await {
        ...
        let request = workspace_rpc::AddWorkspaceRequest { path, surface };
        let response = remote_backend::call_remote(
            &*state,
            app,
            "add_workspace",
