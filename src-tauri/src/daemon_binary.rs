...
pub(crate) fn daemon_binary_candidates() -> &'static [&'static str] {
    if cfg!(windows) {
        &["ladonx_daemon.exe", "ladonx-daemon.exe"]
    } else {
        &["ladonx_daemon", "ladonx-daemon"]
    }
}
...
pub(crate) fn resolve_daemon_binary_path() -> Result<PathBuf, String> {
    ...
    let current_exe = crate::settings::current_exe_path()
        .ok_or_else(|| "Unable to resolve current executable".to_string())?;
    let parent = current_exe
        .parent()
        .ok_or_else(|| "Unable to resolve executable directory".to_string())?;
    let candidate_names = daemon_binary_candidates();

    if let Ok(explicit_raw) = std::env::var("LADONX_DAEMON_PATH") {
