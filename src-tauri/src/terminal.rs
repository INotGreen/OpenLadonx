...
async fn open_terminal_session_with_command(
    workspace_id: String,
    terminal_id: String,
    cols: u16,
    rows: u16,
    ...
    let workspace_cwd = get_workspace_path(&workspace_id, &state).await?;
    ...
    cmd.cwd(cwd_override.unwrap_or(workspace_cwd));
...
#[tauri::command]
pub(crate) async fn claude_code_open(
    workspace_id: String,
    terminal_id: String,
    cols: u16,
    rows: u16,
    ...
    let workspace_cwd = get_workspace_path(&workspace_id, &state).await?;
    let claude_bin =
        crate::bundled_cli::ensure_bundled_claude().unwrap_or_else(|| PathBuf::from("claude"));
    let mut cmd = CommandBuilder::new(claude_bin.to_string_lossy().to_string());
    let claude_home = crate::bundled_cli::bundled_claude_home();
    if let Some(claude_home) = claude_home.as_ref() {
        cmd.env(crate::settings::CLAUDE_HOME_ENV, claude_home);
        cmd.env(crate::settings::CLAUDE_CONFIG_DIR_ENV, claude_home);
    }
    if let Some(api_key) = crate::settings::anthropic_api_key_from_env() {
        cmd.env(crate::settings::ANTHROPIC_API_KEY_ENV, api_key);
    }
    ...
    cmd.env(crate::settings::ANTHROPIC_BASE_URL_ENV, anthropic_base_url);
    cmd.arg("--add-dir");
    cmd.arg(workspace_cwd.to_string_lossy().to_string());
    open_terminal_session_with_command(
        workspace_id,
        terminal_id,
        cols,
