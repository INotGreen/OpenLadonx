...
pub(crate) fn build_codex_path_env(codex_bin: Option<&str>) -> Option<String> {
    ...
    if let Ok(current_exe) = env::current_exe() {
        if let Some(app_dir) = current_exe.parent() {
            extras.push(app_dir.to_path_buf());
        }
    }
    ...
    {
        if let Some(path) = crate::bundled_cli::bundled_cli_path() {
...
pub(crate) fn build_codex_command_with_bin(
    ...
    #[cfg(target_os = "windows")]
    let mut command = {
        let mut bundled_home = None;
        let bin = if codex_bin
            .as_deref()
            .map(str::trim)
            .filter(|value| !value.is_empty())
            .is_none()
        {
            if let Some(bundled) = crate::bundled_cli::bundled_cli_path() {
                bundled_home = crate::bundled_cli::bundled_cli_home();
...
pub(crate) async fn spawn_workspace_session<E: EventSink>(
    ...
    if let Some(path) = codex_home.as_ref() {
        command.env(crate::settings::CODEX_HOME_ENV, path);
    }
