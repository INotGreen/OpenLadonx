...
fn parse_read_binary_file_request(params: &Value) -> Result<ReadBinaryFileRequest, String> {
    serde_json::from_value(params.clone()).map_err(|err| err.to_string())
}
...
pub(super) async fn try_handle(
    ...
    match method {
        ...
        "read_binary_file" => {
            let request = match parse_read_binary_file_request(params) {
                Ok(value) => value,
                Err(err) => return Some(Err(err)),
            };
            Some(
                serialize_result(
                    state.read_binary_file(request.workspace_id, request.relative_path),
                )
                .await,
            )
        }
