...
const APP_IDENTIFIER: &str = "com.ladonx.app";
...
fn load_settings(data_dir: &Path) -> Option<AppSettings> {
    let settings_path = data_dir.join(settings::LADONX_SETTINGS_FILE_NAME);
    storage::read_settings(&settings_path)
        .ok()
        .or_else(|| storage::read_settings(&data_dir.join("settings.json")).ok())
}
