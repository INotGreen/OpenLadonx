...
#[path = "../storage.rs"]
mod storage;
#[path = "ladonx_daemon/transport.rs"]
mod transport;
#[allow(dead_code)]
#[path = "../types.rs"]
mod types;
...
use shared::{
    agents_config_core, claude_history_core, codex_aux_core, codex_core, files_core, git_core,
    git_ui_core, local_usage_core, settings_core, workspaces_core, worktree_core,
};
use storage::{
    apply_codex_api_key_env, read_settings, read_workspaces, read_workspaces_with_history,
};
use types::{
...
const DEFAULT_LISTEN_ADDR: &str = settings::DEFAULT_REMOTE_BACKEND_HOST;
const MAX_IN_FLIGHT_RPC_PER_CONNECTION: usize = 32;
const DAEMON_NAME: &str = "ladonx-daemon";
...
impl DaemonState {
    fn load(config: &DaemonConfig, event_sink: DaemonEventSink) -> Self {
        let program_dir = settings::resolve_runtime_data_dir();
        let storage_path = settings::resolve_workspaces_path(&program_dir, &config.data_dir);
        let settings_path = settings::resolve_settings_path(&program_dir, &config.data_dir);
    ...
    ) -> Result<files_core::BinaryFileResponse, String> {
        files_core::read_binary_file_core(&self.workspaces, &workspace_id, &relative_path).await
    }
    ...
    async fn start_thread(
        ...
        codex_core::start_thread_core(&self.sessions, &self.workspaces, workspace_id, access_mode)
            .await
    }
    ...
    async fn list_threads(
        ...
        if let Some(workspace) = claude_workspace {
            return claude_history_core::list_claude_history_threads_core(
    ...
    async fn model_list(&self, workspace_id: String) -> Result<Value, String> {
        if let Some(models) =
            codex_core::model_list_from_openai_api_core(&self.app_settings).await?
        {
            return Ok(models);
        }
...
fn default_data_dir() -> PathBuf {
    if let Ok(xdg) = env::var("XDG_DATA_HOME") {
        ...
        if !trimmed.is_empty() {
            return PathBuf::from(trimmed).join("ladonx-daemon");
        }
    }
...
fn main() {
    ...
    runtime.block_on(async move {
        ...
        eprintln!(
            "ladonx-daemon listening on {} (data dir: {})",
            config.listen,
            state
                .storage_path
                .parent()
