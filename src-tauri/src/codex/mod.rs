...
use crate::shared::automation_core::{self, AutomationItem};
use crate::shared::claude_history_core;
...
pub(crate) async fn list_threads(
    ...
    if let Some(workspace) = claude_workspace {
        return claude_history_core::list_claude_history_threads_core(&workspace, cursor, limit)
            .await;
    }
...
pub(crate) async fn model_list(
    ...
    if let Some(models) = codex_core::model_list_from_openai_api_core(&state.app_settings).await? {
        return Ok(models);
    }
...
#[tauri::command]
pub(crate) async fn automations_list() -> Result<Vec<AutomationItem>, String> {
    automation_core::list_automations_core(crate::codex::home::resolve_default_codex_home())
}

#[tauri::command]
pub(crate) async fn plugins_marketplace_list(
) -> Result<Vec<crate::shared::plugins_marketplace_core::PluginMarketItem>, String> {
    crate::shared::plugins_marketplace_core::list_plugins_marketplace_core(
        crate::settings::resolve_effective_codex_home(),
    )
}

#[tauri::command]
pub(crate) async fn configured_plugins_list(
) -> Result<Vec<crate::shared::plugins_marketplace_core::ConfiguredPluginItem>, String> {
    crate::shared::plugins_marketplace_core::list_configured_plugins_core(
        crate::settings::resolve_effective_codex_home(),
    )
}

#[tauri::command]
pub(crate) async fn install_plugin(plugin_name: String) -> Result<(), String> {
    crate::codex::plugin_ops::install_plugin_core(plugin_name.as_str()).await
}

#[tauri::command]
pub(crate) async fn uninstall_plugin(plugin_name: String) -> Result<(), String> {
    crate::codex::plugin_ops::uninstall_plugin_core(plugin_name.as_str()).await
}
