...
export default defineConfig(({ command }) => {
  const enableReactInspector = command === "serve";

  return {
    plugins: [
      react({
        babel: enableReactInspector
          ? {
              plugins: [["@react-dev-inspector/babel-plugin", { cwd: projectRoot }]],
            }
          : undefined,
      }),
      ...(enableReactInspector ? [inspectorServer()] : []),
    ],
    resolve: {
      alias: {
