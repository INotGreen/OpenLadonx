// @ts-nocheck - Migrated from App.jsx, types need gradual migration
import {
...
import annotationToolIconRaw from '@/assets/svg-icons/tool-comment.svg?raw'
import {
...
// Global type declarations for canvas editor
declare global {
  interface Window {
    __cowartEditor?: any;
    __cowartSelection?: () => any;
    __cowartViewState?: () => any;
  }
}

const CANVAS_ENDPOINT = "/api/canvas"; // TODO: Define actual endpoint if needed
...
const annotationToolIconSvg = annotationToolIconRaw.replace('black', 'currentColor')
...
function CanvasDropZone() {
  ...
  useEffect(() => {
    const root = document.querySelector('.cowart-canvas')
    if (!root) {
      console.warn('CanvasDropZone: .cowart-canvas not found')
      return
    }
    console.log('CanvasDropZone: attached to', root)
    ...
    async function handleDrop(event) {
      console.log('CanvasDropZone: drop event', event)
      if (!hasFiles(event)) return
      event.preventDefault()
      ...
      if (!editor) {
        console.warn('CanvasDropZone: Editor not available for drop')
        return
      }
      console.log('CanvasDropZone: editor available, files:', event.dataTransfer.files.length)

      const files = Array.from(event.dataTransfer.files)
      if (files.length === 0) return

      const rect = root.getBoundingClientRect()
      const pagePoint = editor.screenToPage({ x: event.clientX - rect.left, y: event.clientY - rect.top })
      ...
      try {
        for (const file of imageFiles) {
          try {
            ...
            console.error('Failed to create image from dropped file:', file.name, error)
          }
        }
      } finally {
    ...
    // Use capture phase to intercept drops before tldraw's React handlers
  ...
  return (
    <div className="cowart-drop-overlay" aria-hidden="true">
      <div className="cowart-drop-overlay-inner">
        {isUploading ? (
          <>
            <span className="cowart-drop-overlay-spinner" />
            <span className="cowart-drop-overlay-text">正在上传…</span>
          </>
        ) : (
          <>
            <svg className="cowart-drop-overlay-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
              <rect x="3" y="3" width="18" height="18" rx="2" />
              <circle cx="8.5" cy="8.5" r="1.5" />
              <path d="M10 14l2-2 5 5" />
            </svg>
            <span className="cowart-drop-overlay-text">释放以添加图片</span>
          </>
        )}
      </div>
    </div>
...
export function CanvasPreviewPanel({ workspacePath, isPanelVisible = true }: CanvasPreviewPanelProps) {
  if (!isPanelVisible) return null;
  const [snapshot, setSnapshot] = useState()
  const [viewState, setViewState] = useState()
  const [loadError, setLoadError] = useState(null)
  const [skippedRecords, setSkippedRecords] = useState([])
  const [isDarkMode, setIsDarkMode] = useState(readCanvasDarkMode)
  const editorRef = useRef(null)

  useEffect(() => {
    const root = document.documentElement
    const mediaQuery = window.matchMedia('(prefers-color-scheme: dark)')
    const syncDarkMode = () => setIsDarkMode(readCanvasDarkMode())
    ...
    return () => {
      ...
      mediaQuery.removeListener?.(syncDarkMode)
    }
  }, [])
    ...
    async function loadCanvas() {
      try {
        const [canvasData, viewStateData] = await Promise.all([
          loadCanvasSnapshot(workspacePath),
          loadViewState(workspacePath)
        ])
        if (cancelled) return
  ...
  const handleMount = useCallback((editor) => {
    ...
    async function syncSelectionState() {
      ...
      try {
        await saveSelection(workspacePath, {
          ...selectionSnapshot,
          updatedAt: new Date().toISOString()
        })
      } catch (error) {
    ...
    async function syncViewState() {
      ...
      try {
        await saveViewState(workspacePath, viewStateSnapshot)
      } catch (error) {
        console.error(error)
      } finally {
    ...
    async function saveCanvas() {
      ...
      isSaving = true
      try {
        await saveCanvasSnapshot(workspacePath, editor.store.getStoreSnapshot())
    ...
    // Live canvas refresh via SSE is not needed in the embedded single-process context.
  ...
  return (
    <main className="cowart-canvas-root" aria-label="Cowart infinite canvas">
      <div className="cowart-canvas">
