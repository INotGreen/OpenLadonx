...
export function useUiScaleShortcuts({
  ...
  useEffect(() => {
    ...
    // Wrap in try-catch to handle cases where Tauri API is not available
    // (e.g., during hot reload or in non-Tauri environments)
    try {
      getCurrentWebview()
        .setZoom(uiScale)
        .catch(() => undefined);
    } catch {
      // Ignore errors when Tauri API is unavailable
    }
  }, [uiScale]);
