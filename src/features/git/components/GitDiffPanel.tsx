...
export function GitDiffPanel({
  ...
  const showLogMenu = useCallback(
    async (event: ReactMouseEvent<HTMLDivElement>, entry: GitLogEntry) => {
      ...
      const copyItem = await MenuItem.new({
        text: String(t("git.menu.copySha")),
        action: async () => {
      ...
      if (githubBaseUrl) {
        const openItem = await MenuItem.new({
          text: String(t("git.menu.openOnGitHub")),
          action: async () => {
    ...
    async (paths: string[]) => {
      ...
      const more =
        paths.length > previewLimit
          ? `\n${String(t("git.menu.moreFiles", { count: paths.length - previewLimit }))}`
          : "";
      const message = isSingle
        ? String(t("git.menu.discardSingleMessage", { path: paths[0] }))
        : String(t("git.menu.discardMultipleMessage", { preview, more }));
      const confirmed = await ask(message, {
        title: String(t("git.menu.discardChanges")),
        kind: "warning",
      });
      if (!confirmed) {
        return;
  ...
  const showFileMenu = useCallback(
    async (
      ...
      if (stagedPaths.length > 0 && onUnstageFile) {
        items.push(
          await MenuItem.new({
            text: String(t("git.menu.unstageFiles", { count: stagedPaths.length })),
            action: async () => {
      ...
      if (unstagedPaths.length > 0 && onStageFile) {
        items.push(
          await MenuItem.new({
            text: String(t("git.menu.stageFiles", { count: unstagedPaths.length })),
            action: async () => {
      ...
      if (targetPaths.length === 1) {
        ...
        items.push(
          await MenuItem.new({
            text: String(t("git.menu.showIn", { app: fileManagerLabel })),
            action: async () => {
              try {
                if (!resolvedRoot && !isAbsolutePathForPlatform(absolutePath)) {
                  pushErrorToast({
                    title: String(t("git.menu.showInErrorTitle", { app: fileManagerLabel })),
                    message: String(t("git.menu.selectGitRootFirst")),
                  });
                  return;
                }
        ...
        items.push(
          await MenuItem.new({
            text: String(t("git.menu.copyFileName")),
            action: async () => {
              await navigator.clipboard.writeText(fileName);
            },
          }),
          await MenuItem.new({
            text: String(t("git.menu.copyFilePath")),
            action: async () => {
      ...
      if (onRevertFile) {
        items.push(
          await MenuItem.new({
            text: String(t("git.menu.discardChangesWithCount", { count: fileCount })),
            action: async () => {
  ...
  return (
    <PanelShell
      ...
      headerRight={
        <div className="git-panel-actions" role="group" aria-label={String(t('git.title'))}>
          <div className="git-panel-select">
            <select
              ...
              aria-label={`${String(t('git.title'))} ${String(t('common.view'))}`}
            >
