...
export function getPerFileDiffEditIdForChange({
  changeIndex,
  diff,
  itemId,
  path,
  ...
  const resolvedPath = diff ? (extractPathFromDiff(diff) ?? normalizedPath) : normalizedPath;
  if (!resolvedPath) {
    return null;
  }
  return `${resolvedPath}@@item-${itemId}@@change-${changeIndex}`;
}
...
export function buildPerFileThreadDiffs(items: ConversationItem[]): {
  ...
  for (const item of items) {
    ...
    for (const [changeIndex, change] of changes.entries()) {
      ...
      const id =
        getPerFileDiffEditIdForChange({
          changeIndex,
          diff,
          itemId: item.id,
          path: change.path,
        }) ?? `${path}@@item-${item.id}@@change-${changeIndex}`;
