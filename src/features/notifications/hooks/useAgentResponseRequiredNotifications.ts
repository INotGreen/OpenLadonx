...
export function useAgentResponseRequiredNotifications({
  ...
  useEffect(() => {
    ...
    const title = String(
      workspaceName
        ? t("notifications.approvalNeededWithWorkspace", { workspaceName })
  ...
  useEffect(() => {
    ...
    const title = String(
      workspaceName
        ? t("notifications.questionWithWorkspace", { workspaceName })
    ...
    const bodyRaw =
      first?.header?.trim() ||
      first?.question?.trim() ||
      String(t("notifications.yourInputNeeded"));
  ...
  const onItemCompleted = useCallback(
    (workspaceId: string, threadId: string, item: Record<string, unknown>) => {
      ...
      const title = String(
        workspaceName
          ? t("notifications.planReadyWithWorkspace", { workspaceName })
          : t("notifications.planReady")
      );
      const text = String(item.text ?? "").trim();
      const body = text
        ? truncateText(text.split("\n")[0] ?? text, MAX_BODY_LENGTH)
        : String(t("notifications.planReadyBody"));
      const extra = {
        kind: "response_required",
        type: "plan",
        workspaceId,
