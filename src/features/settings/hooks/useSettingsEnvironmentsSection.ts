...
import { subscribeEnvInstallStatus } from "@services/events";
import { getEnvRuntimeStatus, startEnvInstall } from "@services/tauri";
...
export const useSettingsEnvironmentsSection = ({
  ...
  const [envRuntimeStatus, setEnvRuntimeStatus] = useState<EnvRuntimeStatus | null>(null);
  const [envRuntimeLoading, setEnvRuntimeLoading] = useState(true);
  ...
  const envInstallBusy = useMemo(
    () =>
      envRuntimeStatus != null &&
      ["pending_elevation", "downloading", "installing"].includes(envRuntimeStatus.install.state),
    [envRuntimeStatus],
  );
  ...
  const refreshEnvRuntimeStatus = useCallback(async () => {
    setEnvRuntimeLoading(true);
    try {
      const next = await getEnvRuntimeStatus();
      setEnvRuntimeStatus(next);
    } finally {
      setEnvRuntimeLoading(false);
    }
  }, []);

  useEffect(() => {
    void refreshEnvRuntimeStatus();
  }, [refreshEnvRuntimeStatus]);

  useEffect(() => {
    return subscribeEnvInstallStatus((status) => {
      setEnvRuntimeStatus((current) =>
        current
          ? {
              ...current,
              install: status,
      ...
      if (["completed", "error", "unsupported"].includes(status.state)) {
        void refreshEnvRuntimeStatus();
      }
    });
  }, [refreshEnvRuntimeStatus]);
  ...
  const handleInstallEnvTarget = useCallback(async (target: EnvInstallTarget) => {
    const next = await startEnvInstall(target);
    setEnvRuntimeStatus((current) =>
      current
        ? {
            ...current,
  ...
  return {
    ...
    onRefreshEnvRuntimeStatus: refreshEnvRuntimeStatus,
    onInstallEnvTarget: handleInstallEnvTarget,
  };
};
