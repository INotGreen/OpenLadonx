...
import { fetchRemoteModelList } from "@/features/models/utils/fetchRemoteModelList";
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      const [modelListResult, configModelResult] = await Promise.allSettled([
        canReadModelList ? fetchRemoteModelList("openai") : Promise.resolve([]),
      ...
      const models = modelsFromList.sort(compareModelsByLatest);
      setState({
        models,
        isLoading: false,
