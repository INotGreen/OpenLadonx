...
const INTERRUPTED_TURN_MARKER_PATTERN =
  /\n?<turn_aborted>\s*The user interrupted the previous turn on purpose\.[\s\S]*?(?:<\/turn_aborted>\s*)?$/i;

export function stripInterruptedTurnMetadata(text: string) {
  if (!text) {
    return "";
  }
  return text.replace(/\r\n/g, "\n").replace(INTERRUPTED_TURN_MARKER_PATTERN, "").trim();
}
...
export function normalizeMessageImageSrc(path: string, workspacePath?: string | null) {
...
export function resolveMessageAssetPath(path: string, workspacePath?: string | null) {
  ...
  const resolved = resolveMessageFileHref(trimmed, workspacePath);
  return resolved?.path ?? trimmed;
}
...
export function buildToolGroups(items: ConversationItem[]): MessageListEntry[] {
  ...
  const flush = () => {
    ...
    const toolCount = normalizedBuffer.reduce((total, item) => {
      if (
        item.kind === "tool" ||
        (item.kind === "reasoning" && item.provider === "claude_code")
      ) {
        return total + 1;
      }
      if (item.kind === "explore") {
...
export function buildToolSummary(
  ...
  if (item.toolType === "claudeCodeToolCall") {
    const toolName = toolNameFromTitle(item.title);
    const normalized = toolName.toLowerCase();
    const targetPath = item.detail || "";
    const fileBase = basename(targetPath);
    const pathDetail =
      targetPath && targetPath !== fileBase ? targetPath : "";
    if (normalized === "read" || normalized === "view_image") {
      return {
        label: "read",
        value: fileBase || toolName || "file",
        detail: pathDetail,
        output: item.output || "",
      };
    }
    ...
    if (normalized === "grep" || normalized === "glob") {
      return {
        label:
          statusToneFromText(item.status) === "processing" ? "searching" : "searched",
        value: targetPath || toolName || "files",
        output: item.output || "",
      };
    }
    return {
      label: "tool",
      value: toolName || item.title || "",
      detail: item.detail || "",
      output: item.output || "",
    };
  }
