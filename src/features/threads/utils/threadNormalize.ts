...
export function mergeTurnPlans(previous: TurnPlan | null, next: TurnPlan | null): TurnPlan | null {
  if (!previous) {
    return next;
  }
  if (!next) {
  ...
  for (const nextStep of next.steps) {
    const index = steps.findIndex((step) => step.step.trim() === nextStep.step.trim());
    if (index >= 0) {
  ...
  const explanation = next.explanation ?? previous.explanation;
...
export function normalizeThreadGoal(threadId: string, goal: Record<string, unknown> | null): ThreadGoal | null {
  if (!goal) {
    return null;
  }
  const objective = asString(goal.objective ?? goal.goal ?? goal.description).trim();
  const status = asString(goal.status).trim();
  return {
    threadId,
    objective: objective || null,
    status: status || null,
    tokensUsed: asFiniteNumber(goal.tokensUsed ?? goal.tokens_used),
    timeUsedSeconds: asFiniteNumber(goal.timeUsedSeconds ?? goal.time_used_seconds),
    createdAt: asFiniteNumber(goal.createdAt ?? goal.created_at),
    updatedAt: asFiniteNumber(goal.updatedAt ?? goal.updated_at),
  };
}

function getGoalRecordFromEvent(entry: unknown): Record<string, unknown> | null {
  ...
  const payload =
    record.payload && typeof record.payload === "object" && !Array.isArray(record.payload)
      ...
      : record;
  const eventType = asString(payload.type ?? payload.method ?? record.type ?? record.method).trim();
  const params =
    payload.params && typeof payload.params === "object" && !Array.isArray(payload.params)
      ? (payload.params as Record<string, unknown>)
      : payload;
  if (eventType !== "thread_goal_updated" && eventType !== "thread/goal/updated") {
    return null;
  }
...
export function extractThreadGoalFromThread(thread: Record<string, unknown> | null | undefined): ThreadGoal | null {
  if (!thread) {
    return null;
  }
  const threadId = asString(thread.id ?? thread.threadId ?? thread.thread_id).trim();
  if (!threadId) {
    return null;
  }
  const directGoal = thread.goal ?? thread.threadGoal ?? thread.thread_goal ?? thread.currentGoal ?? thread.current_goal;
  if (directGoal && typeof directGoal === "object" && !Array.isArray(directGoal)) {
    return normalizeThreadGoal(threadId, directGoal as Record<string, unknown>);
  }
  const eventSources = [thread.events, thread.eventLog, thread.event_log, thread.history];
  for (const source of eventSources) {
    if (!Array.isArray(source)) {
      continue;
    }
    for (let index = source.length - 1; index >= 0; index -= 1) {
      const goal = getGoalRecordFromEvent(source[index]);
      if (goal) {
        return normalizeThreadGoal(threadId, goal);
      }
    }
  }
  return null;
