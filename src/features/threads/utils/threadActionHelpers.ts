...
function isRemoteItemRicherThanLocal(item: ConversationItem, local: ConversationItem) {
  if (item.kind !== local.kind) {
    return true;
  }
  if (item.kind === "message" && local.kind === "message") {
    const remoteImageCount = item.images?.length ?? 0;
    const localImageCount = local.images?.length ?? 0;
    return (
      remoteImageCount > localImageCount ||
      item.text.length > local.text.length
    );
  }
  if (item.kind === "tool" && local.kind === "tool") {
    return (
      (item.output?.length ?? 0) > (local.output?.length ?? 0) ||
      (item.changes?.length ?? 0) > (local.changes?.length ?? 0)
    );
  }
  if (item.kind === "reasoning" && local.kind === "reasoning") {
    return (
      item.summary.length + item.content.length >
      local.summary.length + local.content.length
    );
  }
  return false;
}
...
export function buildResumeHydrationPlan({
  ...
  localItemsAlreadyHydrated?: boolean;
  ...
  const hasRemoteItemsMissingLocally =
    items.length > 0 &&
    localItems.length > 0 &&
    items.some((item) => {
      const local = localItems.find((entry) => entry.id === item.id);
      return !local || isRemoteItemRicherThanLocal(item, local);
    });
  if (
    localItems.length > 0 &&
    localItemsAlreadyHydrated !== false &&
    !replaceLocal &&
    !hasRemoteItemsMissingLocally
  ) {
    return {
