...
import {
  ...
  deleteClaudeCodeStoredChat,
  readThread as readThreadService,
  setClaudeCodeStoredChatTitle,
  setThreadName as setThreadNameService,
} from "@services/tauri";
import {
  makeCustomNameKey,
  purgeThreadPersistence,
  saveCustomName,
...
export function useThreads({
  ...
  const hydrateThreadOnTurnCompletedRef = useRef<
    ((workspaceId: string, threadId: string) => void | Promise<void>) | null
  >(null);
  ...
  const itemSnapshotWriteTimerRef = useRef<number | null>(null);
  ...
  const workspaceSourceByIdRef = useRef<Record<string, WorkspaceInfo["source"]>>({});
  if (activeWorkspace) {
    workspaceSourceByIdRef.current[activeWorkspace.id] = activeWorkspace.source;
  }
  const getWorkspaceSource = useCallback(
    (workspaceId: string) => workspaceSourceByIdRef.current[workspaceId] ?? "codex",
    [],
  );
  ...
  useEffect(() => {
    ...
    if (itemSnapshotWriteTimerRef.current !== null) {
      window.clearTimeout(itemSnapshotWriteTimerRef.current);
    }
    itemSnapshotWriteTimerRef.current = window.setTimeout(() => {
      itemSnapshotWriteTimerRef.current = null;
      Object.entries(state.itemsByThread).forEach(([threadId, items]) => {
        if (items.length === 0) {
          return;
        }
        const workspaceEntry = Object.entries(state.threadsByWorkspace).find(
          ([, threads]) => threads.some((thread) => thread.id === threadId),
        );
        const workspaceId =
          workspaceEntry?.[0] ??
          Object.entries(state.activeThreadIdByWorkspace).find(
            ([, activeThread]) => activeThread === threadId,
          )?.[0] ??
          activeWorkspaceId;
        if (!workspaceId) {
          return;
        }
        saveThreadItemSnapshot(workspaceId, threadId, items);
      });
    }, 500);
    return () => {
      if (itemSnapshotWriteTimerRef.current !== null) {
  ...
  const getInterruptedTurnId = useCallback(
    (threadId: string) => state.interruptedTurnIdByThread[threadId] ?? null,
    [state.interruptedTurnIdByThread],
  );
  ...
  const renameThread = useCallback(
    (workspaceId: string, threadId: string, newName: string) => {
      ...
      const workspaceSource = getWorkspaceSource(workspaceId);
      const persistRename =
        workspaceSource === "claude_code"
          ? setClaudeCodeStoredChatTitle(threadId, workspaceId, newName)
    ...
    [customNamesRef, dispatch, getWorkspaceSource, onDebug],
  );
          ...
          try {
            ...
            dispatch({
              ...
              patch: {
                ...
                ...(summary.filePath ? { filePath: summary.filePath } : {}),
              },
            });
  ...
  const threadHandlers = useThreadEventHandlers({
    ...
    hydrateThreadOnTurnCompleted: (workspaceId, threadId) =>
      hydrateThreadOnTurnCompletedRef.current?.(workspaceId, threadId),
  });
  ...
  hydrateThreadOnTurnCompletedRef.current = async (workspaceId, threadId) => {
    if ((state.activeThreadIdByWorkspace[workspaceId] ?? null) !== threadId) {
      return;
    }
    await resumeThreadForWorkspace(workspaceId, threadId, true, false);
  };
  ...
  const startThreadForWorkspace = useCallback(
    ...
    [dispatch, ensureWorkspaceRuntimeCodexArgsBestEffort, getWorkspaceSource, loadedThreadsRef, startThreadForWorkspaceInternal],
  );
  ...
  const ensureThreadForActiveWorkspace = useCallback(async () => {
    ...
    if (!threadId) {
      ...
      if (activeWorkspace.source === "codex") {
    ...
    (threadId: string | null, workspaceId?: string) => {
      ...
      if (threadId) {
        void (async () => {
          ...
          if (!hasActiveTurnInWorkspace && getWorkspaceSource(targetId) === "codex") {
            await ensureWorkspaceRuntimeCodexArgsBestEffort(targetId, threadId, "resume");
          }
          await resumeThreadForWorkspace(
            targetId,
            threadId,
            hasLocalSnapshot && !hasActiveTurnInWorkspace,
            false,
          );
        })();
      }
  ...
  const removeThread = useCallback(
    async (workspaceId: string, threadId: string) => {
      if (getWorkspaceSource(workspaceId) === "claude_code") {
        await deleteClaudeCodeStoredChat(threadId, workspaceId);
      } else {
        await archiveThread(workspaceId, threadId);
      }
      unpinThread(workspaceId, threadId);
      purgeThreadPersistence(workspaceId, threadId);
      dispatch({ type: "removeThread", workspaceId, threadId });
    },
    [archiveThread, getWorkspaceSource, unpinThread],
  );
  ...
  return {
    ...
    goalByThread: state.goalByThread,
