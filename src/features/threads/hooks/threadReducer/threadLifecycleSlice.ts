  ...
  switch (action.type) {
    ...
    case "ensureThread": {
      ...
      return {
        ...
        activeThreadIdByWorkspace: {
          ...state.activeThreadIdByWorkspace,
          [action.workspaceId]: shouldActivate
    ...
    case "removeThread": {
      ...
      const { [action.threadId]: removedPlan, ...restPlans } = state.planByThread;
      const { [action.threadId]: removedGoal, ...restGoals } = state.goalByThread;
      const { [action.threadId]: removedParent, ...restParents } = state.threadParentById;
      const { [action.threadId]: removedInterruptedTurn, ...restInterruptedTurns } =
        state.interruptedTurnIdByThread;
      void removedPlan;
      void removedGoal;
      void removedParent;
      void removedInterruptedTurn;
      const nextParents = Object.fromEntries(
      ...
      return {
        ...
        goalByThread: restGoals,
        threadParentById: nextParents,
        interruptedTurnIdByThread: restInterruptedTurns,
        activeThreadIdByWorkspace: {
          ...state.activeThreadIdByWorkspace,
          [action.workspaceId]: nextActive,
        },
    ...
    case "setInterruptedTurnId":
      return {
        ...state,
        interruptedTurnIdByThread: {
          ...state.interruptedTurnIdByThread,
          [action.threadId]: action.turnId,
        },
      };
    ...
    case "setThreads": {
      ...
      const appendExistingThread = (thread: ThreadSummary) => {
        if (!thread?.id || hidden[thread.id] || includedIds.has(thread.id)) {
          return;
        }
        reconciled.push(freshenAnchorSummary(thread));
