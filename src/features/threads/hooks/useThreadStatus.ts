...
export function useThreadStatus({ dispatch }: UseThreadStatusOptions) {
  ...
  const setInterruptedTurnId = useCallback(
    (threadId: string, turnId: string | null) => {
      dispatch({ type: "setInterruptedTurnId", threadId, turnId });
    },
    [dispatch],
  );

  return { markProcessing, markReviewing, setActiveTurnId, setInterruptedTurnId };
}
