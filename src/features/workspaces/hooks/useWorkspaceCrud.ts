...
import type { DebugEntry, WorkspaceInfo, WorkspaceSettings, WorkspaceSurface } from "../../../types";
...
function getWorkspaceSurface(workspace: WorkspaceInfo): WorkspaceSurface {
...
function findWorkspaceByPath(
  workspaces: WorkspaceInfo[],
  path: string,
  surface: WorkspaceSurface = "codex",
) {
  const pathKey = normalizeWorkspacePathKey(path);
  return workspaces.find(
    (workspace) =>
      normalizeWorkspacePathKey(workspace.path) === pathKey &&
      getWorkspaceSurface(workspace) === surface,
  ) ?? null;
}
...
export function useWorkspaceCrud({
  ...
  const addWorkspaceFromPath = useCallback(
    async (path: string, options?: { activate?: boolean; surface?: WorkspaceSurface }) => {
      const selection = path.trim();
      if (!selection) {
        return null;
      }
      const shouldActivate = options?.activate !== false;
      const surface = options?.surface ?? "codex";
      const existingWorkspace = findWorkspaceByPath(workspaces, selection, surface);
      ...
      onDebug?.({
        ...
        source: "client",
        label: "workspace/add",
        payload: { path: selection, surface },
      });
      try {
        const workspace = await addWorkspaceService(selection, surface);
        setWorkspaces((prev) =>
          findWorkspaceByPath(prev, workspace.path, surface) ? prev : [...prev, workspace],
        );
        if (shouldActivate) {
  ...
  const updateWorkspaceSettings = useCallback(
    async (workspaceId: string, patch: Partial<WorkspaceSettings>) => {
      ...
      let currentWorkspace = workspaces.find((entry) => entry.id === workspaceId) ?? null;
      let currentSettings =
      ...
      if (!currentWorkspace || !currentSettings) {
        try {
          ...
          currentWorkspace =
            latestWorkspaces.find((entry) => entry.id === workspaceId) ?? null;
          currentSettings =
            currentWorkspace?.settings ??
            workspaceSettingsRef.current.get(workspaceId) ??
            null;
        } catch (error) {
          onDebug?.({
            id: `${Date.now()}-client-update-workspace-settings-refresh-error`,
            timestamp: Date.now(),
            source: "error",
            label: "workspace/settings refresh error",
            payload: error instanceof Error ? error.message : String(error),
          });
        }
      }
