...
function positionInsideRect(
  position: { x: number; y: number },
  rect: DOMRect,
) {
  return pointInsideRect(normalizeViewportPosition(position), rect);
}
...
export function useWorkspaceDropZone({
  ...
  useEffect(() => {
    ...
    unlisten = subscribeWindowDragDrop((event) => {
      ...
      if (payload.type === "over" || payload.type === "enter") {
        ...
        const isInside = positionInsideRect(position, rect);
  ...
  const handleDrop = (event: DragEvent<HTMLElement>) => {
    ...
    setIsDragOver(false);
    lastClientPositionRef.current = null;
    const paths = extractPathsFromTransferData(event.dataTransfer);
    emitPaths(paths);
  };
