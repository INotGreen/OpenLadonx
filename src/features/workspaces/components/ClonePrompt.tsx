...
export function ClonePrompt({
  ...
  return (
    <ModalShell
      ...
      ariaLabel={String(t('workspace.clonePrompt.title'))}
      onBackdropClick={() => {
        if (!isBusy) {
          onCancel();
        }
    ...
    >
      <div className="ds-modal-title clone-modal-title">{String(t('workspace.clonePrompt.title'))}</div>
      <div className="ds-modal-subtitle clone-modal-subtitle">
        {String(t('workspace.clonePrompt.subtitle', { name: workspaceName }))}
      </div>
      <label className="ds-modal-label clone-modal-label" htmlFor="clone-copy-name">
        {String(t('workspace.clonePrompt.copyName'))}
      </label>
      <input
        id="clone-copy-name"
        ref={inputRef}
      ...
      <label className="ds-modal-label clone-modal-label" htmlFor="clone-copies-folder">
        {String(t('workspace.clonePrompt.copiesFolder'))}
      </label>
      <div className="clone-modal-folder-row">
        <textarea
          ...
          value={copiesFolder}
          placeholder={String(t('workspace.clonePrompt.notSet'))}
          readOnly
          rows={1}
          wrap="off"
          onFocus={(event) => {
        ...
        <button
          ...
          disabled={isBusy}
        >
          {String(t('workspace.clonePrompt.choose'))}
        </button>
        <button
          type="button"
          className="ghost clone-modal-button"
        ...
        >
          {String(t('workspace.clonePrompt.clear'))}
        </button>
      </div>
      {showSuggested && (
        <div className="clone-modal-suggested">
          <div className="clone-modal-suggested-label">{String(t('workspace.clonePrompt.suggested'))}</div>
          <div className="clone-modal-suggested-row">
            <textarea
              ...
              rows={1}
              wrap="off"
              aria-label={String(t('workspace.clonePrompt.suggested'))}
            ...
            <button
              ...
              disabled={isBusy}
            >
              {String(t('workspace.clonePrompt.useSuggested'))}
            </button>
          </div>
        </div>
      )}
      ...
      <div className="ds-modal-actions clone-modal-actions">
        <button
          ...
          disabled={isBusy}
        >
          {String(t('workspace.clonePrompt.cancel'))}
        </button>
        <button
        ...
        >
          {String(t('workspace.clonePrompt.create'))}
        </button>
      </div>
    </ModalShell>
  );
