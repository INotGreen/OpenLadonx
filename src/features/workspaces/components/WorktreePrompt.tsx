...
export function WorktreePrompt({
  ...
  return (
    <ModalShell
      ...
      ariaLabel={String(t("workspace.newWorktreeAgent"))}
      onBackdropClick={() => {
        if (!isBusy) {
          onCancel();
        }
    ...
    >
      <div className="ds-modal-title worktree-modal-title">{String(t("workspace.newWorktreeAgent"))}</div>
      <div className="ds-modal-subtitle worktree-modal-subtitle">
        {String(t("workspace.createWorktreeUnder"))} "{workspaceName}".
      </div>
      <label className="ds-modal-label worktree-modal-label" htmlFor="worktree-name">
        {String(t("workspace.name"))}
      </label>
      <input
        id="worktree-name"
        ref={inputRef}
        ...
        value={name}
        placeholder={String(t("workspace.optional"))}
          ...
          <BranchList
            ...
            emptyText={
              branch.trim().length > 0
                ? String(t("workspace.noMatchingBranches"))
                : String(t("workspace.noBranchesFound"))
            }
            onMouseEnter={(index) => {
              setDidNavigateBranches(true);
      ...
      <div className="worktree-modal-checkbox-row">
        ...
        <label className="worktree-modal-checkbox-label" htmlFor="worktree-copy-agents">
          <span dangerouslySetInnerHTML={{ __html: String(t("workspace.copyAgentsMdIntoWorktree")) }} />
        </label>
      </div>
      <div className="ds-modal-divider worktree-modal-divider" />
      <div className="worktree-modal-section-title">{String(t("workspace.environmentSetupScript"))}</div>
      <div className="worktree-modal-hint">
        {String(t("workspace.environmentSetupScriptHint"))}
      </div>
      <textarea
