...
export function WorkspaceHomeRunControls({
  ...
  return (
    <div className="workspace-home-controls">
      <div className="workspace-home-controls-row">
        {showRunMode && (
          <SplitActionMenu
            containerRef={runModeRef}
        ...
        <div className="composer-select-wrap workspace-home-control">
          <div className="open-app-button">
            ...
            <select
              ...
              aria-label={String(t("composer.labels.intelligence"))}
              value={selectedEffort ?? ""}
              onChange={(event) => onSelectEffort(event.target.value)}
              disabled={isSubmitting || !reasoningSupported}
            >
              {reasoningOptions.length === 0 && (
                <option value="">{formatReasoningEffortLabel(t, null)}</option>
              )}
              {reasoningOptions.map((effortOption) => (
                <option key={effortOption} value={effortOption}>
                  {formatReasoningEffortLabel(t, effortOption)}
                </option>
              ))}
            </select>
          </div>
        ...
        {agentEditor && (
          <label className="workspace-home-agent-toggle">
            <input
              type="checkbox"
              checked={showAgentEditor}
              onChange={(event) => setShowAgentEditor(event.target.checked)}
            />
            <span>AGENTS.md</span>
          </label>
        )}
      </div>
      {agentEditor && showAgentEditor && (
        <div className="workspace-home-agent">{agentEditor}</div>
      )}
    </div>
  );
}
