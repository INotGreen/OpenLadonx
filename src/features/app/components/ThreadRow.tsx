...
export const ThreadRow = memo(function ThreadRow({
  ...
  const statusLabel =
    statusClass === "reviewing"
      ? String(t("sidebar.threadRow.reviewing"))
      : hasPendingUserInput
        ? String(t("sidebar.threadRow.waiting"))
        : null;
  const subagentLabel =
  ...
  return (
    <div
      className={`thread-row ${
        ...
        depth > 0 ? " is-nested" : ""
      }${isPinned ? " is-pinned" : ""}${className ? ` ${className}` : ""}`}
      style={indentStyle}
      onClick={() => onSelectThread(workspaceId, thread.id)}
      onContextMenu={(event) =>
        onShowThreadMenu(event, workspaceId, thread.id, canPin, thread.filePath)
      }
      role="button"
      tabIndex={0}
      onKeyDown={(event) => {
    ...
    >
      {/* <ChatGptMark processing={isStatusActive} /> */}
        ...
        {hasDetails && (
          <div className="thread-details">
            ...
            {showPinnedLabel && isPinned && (
              <span className="thread-pinned-label">
                {String(t("sidebar.threadRow.pinned"))}
              </span>
            )}
          </div>
        )}
      ...
      <div className="thread-meta">
        {canToggleSubagents ? (
          <button
            ...
            aria-label={
              subagentsExpanded
                ? String(t("sidebar.threadRow.hideSubagents"))
                : String(t("sidebar.threadRow.showSubagents"))
            }
            aria-expanded={subagentsExpanded}
          >
            <span className="thread-subagent-time-label">
              {relativeTime ?? String(t("sidebar.threadRow.now"))}
            </span>
          ...
          <>
            {isProcessing ? (
              <span
                className="thread-time thread-time-loading"
                title={String(t("sidebar.threadRow.running"))}
              >
                <LoaderCircle className="thread-time-spinner" aria-hidden />
              </span>
            ) : (
