...
function formatResetLabel(resetsAt?: number | null) {
  ...
  return String(i18n.t("settings.usageResets", { relative }));
}
...
function formatCreditsLabel(accountRateLimits: RateLimitSnapshot | null) {
  ...
  if (credits.unlimited) {
    return String(i18n.t("settings.availableCreditsUnlimited"));
  }
  ...
  if (Number.isFinite(intValue) && intValue > 0) {
    return String(i18n.t("settings.availableCreditsValue", { value: intValue }));
  }
  const floatValue = Number.parseFloat(balance);
  if (Number.isFinite(floatValue) && floatValue > 0) {
    const rounded = Math.round(floatValue);
    return rounded > 0
      ? String(i18n.t("settings.availableCreditsValue", { value: rounded }))
      : null;
  }
  return null;
}
