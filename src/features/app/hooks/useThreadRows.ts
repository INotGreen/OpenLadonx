...
export function useThreadRows(threadParentById: Record<string, string>) {
  ...
  const getThreadRows = useCallback(
    (
      ...
      // Keep subagent threads visible even if their parent mapping has not been
      // rehydrated yet. We can still render them as roots until the linkage
      // arrives, which avoids whole thread groups disappearing after refreshes.
      ...
      const visibleRoots = unpinnedRoots;
