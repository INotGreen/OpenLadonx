...
import type { DebugEntry, WorkspaceInfo, WorkspaceSurface } from "../../../types";

type Params = {
  isCompact: boolean;
  addWorkspace: (surface?: WorkspaceSurface) => Promise<WorkspaceInfo | null>;
  ...
  addWorkspacesFromPaths: (paths: string[], surface?: WorkspaceSurface) => Promise<WorkspaceInfo | null>;
...
export function useWorkspaceActions({
  ...
  const handleAddWorkspace = useCallback(async (surface?: WorkspaceSurface) => {
    try {
      const workspace = await addWorkspace(surface);
      if (workspace) {
