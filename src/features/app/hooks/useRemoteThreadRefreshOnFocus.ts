...
export function useRemoteThreadRefreshOnFocus({
  ...
  const safeUnlisten = (unlisten: (() => void) | null) => {
    if (!unlisten) {
      return;
    }
    try {
      ...
      console.warn("[remote-thread-refresh] failed to unlisten window event", error);
    }
  };
  ...
  useEffect(() => {
    ...
    return () => {
      didCleanup = true;
      safeUnlisten(unlistenWindowFocus);
      safeUnlisten(unlistenWindowBlur);
