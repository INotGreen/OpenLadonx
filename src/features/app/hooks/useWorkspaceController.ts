...
import type { AppSettings, WorkspaceInfo, WorkspaceSurface } from "../../../types";
...
export function useWorkspaceController({
  ...
  const runAddWorkspacesFromPaths = useCallback(
    async (
      paths: string[],
      options?: { rememberMobileRemoteRecents?: boolean; surface?: WorkspaceSurface },
    ) => {
      const result = await addWorkspacesFromPathsCore(paths, { surface: options?.surface });
  ...
  const addWorkspacesFromPaths = useCallback(
    async (paths: string[], surface?: WorkspaceSurface): Promise<WorkspaceInfo | null> => {
      const result = await runAddWorkspacesFromPaths(paths, { surface });
      return result.firstAdded;
    },
    [runAddWorkspacesFromPaths],
  );

  const addWorkspace = useCallback(async (surface?: WorkspaceSurface): Promise<WorkspaceInfo | null> => {
