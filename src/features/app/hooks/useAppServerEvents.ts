  ...
  useEffect(() => {
    const unlisten = subscribeAppServerEvents((payload) => {
      ...
      if (method === "thread/goal/updated") {
        ...
        const goal = goalRaw && typeof goalRaw === "object" && !Array.isArray(goalRaw)
          ? (goalRaw as Record<string, unknown>)
          : null;
        const threadId = String(
          params.threadId ?? params.thread_id ?? goal?.threadId ?? goal?.thread_id ?? "",
        );
        if (threadId) {
          currentHandlers.onThreadGoalUpdated?.(workspace_id, threadId, goal);
        }
        return;
      }
