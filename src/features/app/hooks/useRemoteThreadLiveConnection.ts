...
function safeUnlisten(
  ...
  try {
    ...
    console.warn(`[remote-thread-live] failed to unlisten ${label}`, error);
  }
}
...
export function useRemoteThreadLiveConnection({
  ...
  useEffect(() => {
    ...
    return () => {
      safeUnlisten(unlisten, "app-server");
    };
  ...
  useEffect(() => {
    ...
    try {
      ...
      windowHandle
        ...
        .then((unlisten) => {
          if (didCleanup) {
            safeUnlisten(unlisten, "window-focus");
            return;
          }
      ...
      windowHandle
        ...
        .then((unlisten) => {
          if (didCleanup) {
            safeUnlisten(unlisten, "window-blur");
            return;
          }
    ...
    return () => {
      didCleanup = true;
      safeUnlisten(unlistenWindowFocus, "window-focus");
      safeUnlisten(unlistenWindowBlur, "window-blur");
