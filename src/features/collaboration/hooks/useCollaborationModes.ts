...
const BUILTIN_ACTION_MODES: CollaborationModeOption[] = [
  {
    id: "plan",
    label: "计划",
    mode: "plan",
    ...
    value: { id: "plan", label: "计划", mode: "plan", builtin: true },
  },
  {
    id: "goal",
    label: "目标",
    ...
    reasoningEffort: null,
    developerInstructions: null,
    value: { id: "goal", label: "目标", mode: "goal", builtin: true },
  },
];

function getModeKey(mode: Pick<CollaborationModeOption, "id" | "mode" | "label">) {
  return (mode.mode || mode.id || mode.label).trim().toLowerCase();
}

function isActionMode(mode: Pick<CollaborationModeOption, "id" | "mode" | "label">) {
  const key = getModeKey(mode);
  return key === "plan" || key === "goal" || key === "计划" || key === "目标";
}

function appendBuiltinActionModes(modes: CollaborationModeOption[]) {
  const existingKeys = new Set(
    modes.flatMap((mode) =>
      [mode.id, mode.mode, mode.label]
        .map((value) => value.trim().toLowerCase())
        .filter(Boolean),
    ),
  );
  const missingBuiltins = BUILTIN_ACTION_MODES.filter(
    (mode) => !existingKeys.has(mode.id) && !existingKeys.has(mode.mode),
  );
  return missingBuiltins.length ? [...modes, ...missingBuiltins] : modes;
}
...
function pickWorkspaceDefaultModeId(modes: CollaborationModeOption[]): string | null {
  return (
    ...
    modes.find((mode) => !isActionMode(mode))?.id ??
    null
  );
}
...
export function useCollaborationModes({
  ...
  const refreshModes = useCallback(async () => {
    ...
    try {
      ...
      const data = appendBuiltinActionModes(rawData
        .map((item: any) => {
