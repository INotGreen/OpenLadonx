...
export const Composer = memo(function Composer({
  ...
  onAttachImages: _onAttachImages,
  ...
  const canSend = text.trim().length > 0 || _attachedImages.length > 0;
  ...
  const {
    ...
    setSelectionStart,
    onSendAutocompleteItem: (_item, insertedText) => {
      sendComposerText(insertedText, defaultSubmitIntent);
    },
    onSkillTrigger: () => {
  ...
  const {
    ...
    historyKey,
    text,
    hasAttachments: _attachedImages.length > 0,
    disabled,
  ...
  const sendComposerText = useCallback((
    ...
    if (!finalText && _attachedImages.length === 0) {
      return;
    }
    if (finalText) {
      recordHistory(finalText);
    }
    const resolvedMentions = resolveBoundAppMentions(finalText, appMentionBindings);
    if (resolvedMentions.length > 0) {
      onSend(finalText, _attachedImages, resolvedMentions, submitIntent);
    } else {
      onSend(finalText, _attachedImages, undefined, submitIntent);
    }
    resetHistoryNavigation();
    setComposerText("");
  ...
  const handleSend = useCallback(
    (submitIntent: ComposerSendIntent = "default") => {
      sendComposerText(text, submitIntent);
    },
    ...
    <footer className={`composer${disabled ? " is-disabled" : ""}`}>
      ...
      <div className="composer-input-stack">
        ...
        <ComposerInput
          ...
          onAddAttachment={_onPickImages}
          onAttachImages={_onAttachImages}
          onRemoveAttachment={_onRemoveImage}
          onPreviewAttachment={_onPreviewAttachment}
