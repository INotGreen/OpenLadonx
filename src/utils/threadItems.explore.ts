...
const MIN_CONTEXT_ITEMS_WHEN_LIMITED = 40;
...
function hasLaterUserMessageWithText(
  ...
  for (let index = startIndex + 1; index < items.length; index += 1) {
    ...
    if (
      item.kind === "message" &&
      item.role === "user" &&
      normalizeUserMessageText(item.text) === text
    ) {
      return true;
    }
  }
...
function selectLimitedThreadItems(items: ConversationItem[], maxItemsPerThread: number) {
  if (items.length <= maxItemsPerThread) {
    return items;
  }
  const tail = items.slice(-maxItemsPerThread);
  const hasTextContext = tail.some(
    (item) =>
      item.kind === "message" ||
      item.kind === "reasoning" ||
      item.kind === "userInput" ||
  ...
  const contextTarget = Math.min(
    MIN_CONTEXT_ITEMS_WHEN_LIMITED,
    Math.max(1, Math.floor(maxItemsPerThread / 2)),
  );
  const context = items
    .slice(0, -maxItemsPerThread)
    .filter(
      (item) =>
        item.kind === "message" ||
        item.kind === "reasoning" ||
  ...
  const tailBudget = Math.max(0, maxItemsPerThread - context.length);
  const balancedTail = tail.slice(-tailBudget);
  return [...context, ...balancedTail];
}
...
export function prepareThreadItems(
  ...
  for (let sourceIndex = 0; sourceIndex < items.length; sourceIndex += 1) {
    ...
    if (item.kind === "message" && item.role === "user") {
      const text = normalizeUserMessageText(item.text);
      if (text) {
        const previousSourceItem = items[sourceIndex - 1];
        const previousUserText = isUserMessage(previousSourceItem)
          ? normalizeUserMessageText(previousSourceItem.text)
          : "";
        if (
          previousUserText &&
          previousUserText !== text &&
          hasLaterUserMessageWithText(items, sourceIndex, text)
        ) {
          continue;
        }
        ...
        for (let index = filtered.length - 1; index >= 0; index -= 1) {
          ...
          if (
            entry.kind === "message" &&
            entry.role === "user" &&
            normalizeUserMessageText(entry.text) === text
          ) {
  ...
  const limited =
    maxItemsPerThread === null
      ? normalized
      : selectLimitedThreadItems(normalized, maxItemsPerThread);
