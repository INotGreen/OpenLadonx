...
function messageMergeKey(item: ConversationItem) {
...
export function mergeThreadItems(
  ...
  const consumedRemoteMessageCounts = new Map<string, number>();
  const merged = remoteItems.map((item) => {
    const local = localItemsById.get(item.id);
    if (local) {
      const key = messageMergeKey(item);
      if (key) {
        consumedRemoteMessageCounts.set(
          key,
          (consumedRemoteMessageCounts.get(key) ?? 0) + 1,
        );
      }
      return chooseRicherItem(item, local);
    }
    return item;
  });
  localItems.forEach((item) => {
    ...
    const key = messageMergeKey(item);
    if (key) {
      const consumed = consumedRemoteMessageCounts.get(key) ?? 0;
      const available = remoteMessageCounts.get(key) ?? 0;
      if (consumed < available) {
        consumedRemoteMessageCounts.set(key, consumed + 1);
        return;
      }
    }
    merged.push(item);
