...
const FILE_LOCATION_LINE_LABEL_PATTERN = /^(.*?)\s*\(\s*line\s+(\d+)(?::(\d+))?\s*\)$/i;
const FILE_LOCATION_CN_LINE_LABEL_PATTERN = /^(.*?)\s*[（(]?\s*(?:第\s*)?(\d+)\s*行\s*[）)]?$/i;
const FILE_URL_LOCATION_HASH_PATTERN = /^#L(\d+)(?:C(\d+))?$/i;

export const FILE_LINK_SUFFIX_SOURCE =
  "(?:(?::\\d+(?::\\d+)?|:\\d+-\\d+)|(?:#L\\d+(?:C\\d+)?)|(?:\\s*\\(\\s*line\\s+\\d+(?::\\d+)?\\s*\\))|(?:\\s*[（(]?\\s*(?:第\\s*)?\\d+\\s*行\\s*[）)]?))?";
...
export function parseFileLocation(rawPath: string): ParsedFileLocation {
  ...
  const lineLabelMatch = trimmed.match(FILE_LOCATION_LINE_LABEL_PATTERN);
  if (lineLabelMatch) {
    const [, path, lineValue, columnValue] = lineLabelMatch;
    const line = parsePositiveInteger(lineValue);
    if (line !== null) {
      return {
        path,
  ...
  const cnLineLabelMatch = trimmed.match(FILE_LOCATION_CN_LINE_LABEL_PATTERN);
  if (cnLineLabelMatch) {
    const [, path, lineValue] = cnLineLabelMatch;
    const line = parsePositiveInteger(lineValue);
    if (line !== null) {
      return {
        path,
