...
import type { ClaudeCodeStreamEvent } from "./tauri";
import type {
  EnvInstallStatus,
...
function safeUnsubscribe(unlisten: Unsubscribe | null | undefined) {
  if (!unlisten) {
    return;
  }
  try {
    ...
    console.warn("[events] failed to unlisten", error);
  }
}
...
function createLocalEventHub<T>() {
  ...
  const emit = (payload: T) => {
    for (const listener of listeners) {
      try {
        ...
        console.error("[events] local listener failed", error);
      }
    }
  };
...
const appUpdateDownloadHub = createEventHub<AppUpdateDownloadStatus>("app-update-download");
const envInstallHub = createEventHub<EnvInstallStatus>("env-install-status");
...
const claudeCodeEventHub = createEventHub<ClaudeCodeStreamEvent>("claude-code-event");
...
const ladonxWechatAuthHub = createEventHub<{
  attemptId: string;
  type: "qrcode" | "success" | "timeout" | "error";
  message?: string;
  appid?: string;
  redirectUri?: string;
  scope?: string;
...
const ladonxAuthChangedHub = createEventHub<void>("ladonx-auth-changed");
const ladonxLocalAuthChangedHub = createLocalEventHub<void>();
const ladonxLoginRequestedHub = createLocalEventHub<void>();
...
export function notifyLadonxAuthChanged(): void {
  ladonxLocalAuthChangedHub.emit(undefined);
}

export function notifyLadonxLoginRequested(): void {
  ladonxLoginRequestedHub.emit(undefined);
}

export function subscribeLadonxWechatAuth(
  onEvent: (event: {
    attemptId: string;
    type: "qrcode" | "success" | "timeout" | "error";
    message?: string;
  ...
  return ladonxWechatAuthHub.subscribe(onEvent, options);
}

export function subscribeLadonxAuthChanged(onEvent: () => void): Unsubscribe {
  const offLocal = ladonxLocalAuthChangedHub.subscribe(() => {
    onEvent();
  });
  const offGlobal = ladonxAuthChangedHub.subscribe(() => {
    onEvent();
  });
  return () => {
    offLocal();
...
export function subscribeLadonxLoginRequested(onEvent: () => void): Unsubscribe {
  return ladonxLoginRequestedHub.subscribe(() => {
    onEvent();
  });
}
...
export function subscribeAppUpdateDownload(
  onEvent: (event: AppUpdateDownloadStatus) => void,
  options?: SubscriptionOptions,
): Unsubscribe {
  return appUpdateDownloadHub.subscribe(onEvent, options);
}

export function subscribeEnvInstallStatus(
  onEvent: (event: EnvInstallStatus) => void,
  options?: SubscriptionOptions,
): Unsubscribe {
  return envInstallHub.subscribe(onEvent, options);
}
...
export function subscribeClaudeCodeEvents(
  onEvent: (event: ClaudeCodeStreamEvent) => void,
  options?: SubscriptionOptions,
): Unsubscribe {
  return claudeCodeEventHub.subscribe(onEvent, options);
}
