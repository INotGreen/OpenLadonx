# LadonX iOS Remote Blueprint (Tailscale + TCP)

This document is the canonical runbook for iOS remote usage with a desktop-hosted LadonX backend over Tailscale.
...
1. Desktop LadonX hosts the daemon and executes Codex workflows.
2. iOS LadonX connects to the desktop daemon using `remoteBackendHost` + token.
...
- Desktop LadonX is installed and able to run local workspaces.
