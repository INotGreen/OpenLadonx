...
Keep LadonX's Agents settings behavior in sync with upstream Codex (`../Codex`) whenever Codex changes:
...
- When users report a mismatch between LadonX settings and Codex runtime behavior
...
## LadonX Files To Update If Upstream Changes
...
- Keep LadonX scoped to the new key only (no legacy alias read/write).
- Keep LadonX writes aligned with upstream expectations.
...
- Reconcile LadonX guardrails when upstream changes.
...
- LadonX default `agents.max_depth` is `1`.
- LadonX currently enforces a product cap of `12` for `agents.max_threads` and `4` for `agents.max_depth` in UI + backend.
