...
- Which app-server events LadonX supports right now.
- Which app-server requests LadonX sends right now.
- Where to look in LadonX to add support.
...
3. Compare Codex events vs LadonX routing.
4. Compare Codex client request methods vs LadonX outgoing request methods.
5. Compare Codex server request methods vs LadonX inbound request handling.
6. Update supported and missing lists below.

Related project skill:
- `.codex/skills/app-server-events-sync/SKILL.md`

## Where To Look In LadonX
...
These are the current Codex v2 `ServerNotification` methods that LadonX
...
## Additional Stream Methods Handled In LadonX
...
- `codex/backgroundThread` (LadonX synthetic bridge event)
- `codex/connected` (LadonX synthetic bridge event)
...
## Supported Requests (LadonX -> App-Server, v2)

These are v2 request methods LadonX currently sends to Codex app-server:
...
Compared against Codex v2 request methods, LadonX currently does not send:
...
## Server Requests (App-Server -> LadonX, v2)
...
4. List LadonX outgoing requests:
...
6. Check for camelCase vs snake_case mismatches:
   ...
   - LadonX generally defends against this by checking both forms (for example in `threadNormalize.ts` and `useAppServerEvents.ts`), while centralizing method/type parsing in `appServerEvents.ts`.
...
- Steering behavior while a turn is processing:
  - LadonX attempts `turn/steer` only when steer capability is enabled, the thread is processing, and an active turn id exists.
  - If `turn/steer` fails, LadonX does not fall back to `turn/start`; it clears stale processing/turn state when applicable, surfaces an error, and returns `steer_failed`.
